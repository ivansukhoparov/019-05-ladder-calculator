...
const drawingAreaSide = document.querySelector('#side-view');
const drawingAreaFront = document.querySelector('#front-view');
const blueprintSide = document.createElementNS(ns, 'svg');
const blueprintFront = document.createElementNS(ns, 'svg');
...
const createSupportSide = function (path) {
	const support = document.createElementNS(ns, 'polygon');
...
const createLine = function (x1, y1, x2, y2, area) {
	const board = document.createElementNS(ns, 'line');
...
const renderSideBP = function (ladderHeight) {
	...
	const xBoard = basePoint[0] - BASE_DEPTH / 5;
	const yBoard = basePoint[1] - BASE_HEIGHT - BASE_HEIGHT / 5;
	const xOffsetSecondBoard = BASE_DEPTH / 1.66;
	//  draw two decking boards for each step
	for (let i = 0; i <= ladderHeight; i++) {
		const xOffset = BASE_DEPTH * i;
		const yOffset = BASE_HEIGHT * i;
		const lastOffset = BASE_HEIGHT / 5;
...
const renderFrontBP = function (ladderHeight, ladderLength) {
	...
	for (let i = -1; i < supportsCount; i++) {
		...
		const x1 = basePoint[0] + xOffset + BASE_HEIGHT / 2 + BASE_WIDTH / 2;
		const x2 = x1 + supportsOffset - BASE_HEIGHT;

		const y1 = basePoint[1] + BASE_HEIGHT / 20;
		const y2 = y1;
